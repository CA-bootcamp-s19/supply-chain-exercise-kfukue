...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  modifier vierfyOwner(){ require(msg.sender == owner, 'msg sender is not owner'); _;}
  ...
  { 
    require(items[_sku].seller != address(0), 'For Sale Modifier seller is 0');
    require(items[_sku].state == State.ForSale, 'State is not ForSale');
    _;
  }
  modifier sold(uint _sku)
  { 
    require(items[_sku].seller != address(0), 'Sold Modifier seller is 0');
    require(items[_sku].state == State.Sold, 'State is not Sold');
    _;
  }
  modifier shipped(uint _sku)
  { 
    require(items[_sku].seller != address(0), 'Shipped Modifier seller is 0');
    require(items[_sku].state == State.Shipped, 'State is not Shipped');
    _;
  }
  modifier received(uint _sku)
  { 
    require(items[_sku].seller != address(0), 'Received Modifier seller is 0');
